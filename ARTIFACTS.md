...
### A11 — Spam Detection Engine
- **Type:** Scoring classifier
- **Path:** `server/src/spam/`
- **Algorithm:** Score-based heuristic — SPF/DKIM signals, header anomalies, keyword lists, structural indicators
- **Action:** Configurable threshold; messages above threshold quarantined to Spam mailbox or rejected at SMTP DATA
- **Tests:** 24 unit tests (score thresholds, header parsing, keyword detection, edge cases)

### A12 — End-to-End Integration Test
- **Type:** Integration test + proof screenshots
- **Path:** `server/src/e2e.rs` (feature-gated: `tests`)
- **Coverage:** Full protocol stack — SMTP delivery, IMAP retrieval, HTTP TOTP login, REST API, webmail SPA
- **Method:** In-process harness on ephemeral ports; headless Chromium screenshots (login, TOTP verify, inbox, message read); PDF report exported and pushed to Mac Mini
- **Deep-link support:** `?partial_token=TOKEN` on `/login` routes straight to TOTP verify step
- **Test-only route:** `/test/inject-session` sets session cookie + redirects (compiled out in release)
...
| Spam / unsolicited mail | Score-based classifier; configurable quarantine or SMTP rejection threshold |
...
```
test result: ok. 156 passed; 0 failed; 0 ignored; 0 measured (server)
...
| 2026-05-18 | `fba5bff` | Add score-based spam detection engine; 156 server tests total |
...
*Updated 2026-05-18 to reflect spam engine, SDVOSB brand, e2e integration test, and full TOI/POA session record (163 tests).*
